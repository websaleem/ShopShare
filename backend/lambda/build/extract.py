...
ALLOWED_ORIGINS = {
    ...
    "https://websaleem.com",
    "http://localhost:8000",   # local dev only — remove in production
}
...
PROMPT = (
    "Extract the shop name and all the line items and their prices from this invoice/receipt. "
    "Return ONLY a valid JSON object. "
    'The object must have exactly two keys: "shopName" (string) and "items" (array of objects). '
...
def _handle_extract(event):
    ...
    if isinstance(items_or_job, dict) and items_or_job.get("jobId"):
        logger.info("[EXTRACT] PDF async job started in %.0fms, jobId=%s", elapsed, items_or_job["jobId"])
...
def _check_rate_limit(user_id: str, action: str, max_count: int) -> bool:
    ...
    try:
        ...
        logger.exception("[RATE_LIMIT] Failed to check rate limit, allowing request")
        return True  # fail open — don't block users if rate-limit infra has issues
def _handle_share(event):
    ...
    try:
        ...
        USER_POOL_ID = os.environ.get("USER_POOL_ID", "your-region_YourPoolId")
        ...
        if target_sub:
            ...
            try:
                ...
                for item in items:
                    shared_items.append({
                        "Item": f"{item.get('Item', 'Item')} (Shared)",
                        "Price": item.get("Price", 0),
                        "BelongsTo": item.get("BelongsTo", "Unassigned")
                    })

                # Try to append to existing state.items list
                try:
                    resp = table.get_item(Key={"userId": target_sub})
                    target_state = resp.get("Item", {}).get("state", {})
                    ...
                    table.put_item(Item={"userId": target_sub, "state": target_state})
                    dynamo_success = True
                except Exception:
        ...
        try:
            ...
            ses_from = os.environ.get("SES_FROM_EMAIL", "noreply@shopshare.com")
